...
public class GoogleloginActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) { //앱이 실행될때 처음 수행되는 곳
        ...
        Toast.makeText(GoogleloginActivity.this, "화면 진입", Toast.LENGTH_SHORT).show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) { //구글 로그인 인증을 요청했을 때 결과 값을 되돌려 받는 곳
        ...
        if(requestCode == REQ_SIGN_GOOGLE) {
            ...
            Toast.makeText(GoogleloginActivity.this, "버튼 클릭", Toast.LENGTH_SHORT).show();
            if(result.isSuccess()) { //인증결과가 성공적이면..
                GoogleSignInAccount account = result.getSignInAccount(); //account라는 데이터는 구글 로그인 정보를 담고 있습니다.(닉네임,프로필사진url,이메일주소..등)
                Toast.makeText(GoogleloginActivity.this, "resultLogin 진입 전", Toast.LENGTH_SHORT).show();
    ...
    private void resultLogin(GoogleSignInAccount account) {
        ...
        Toast.makeText(GoogleloginActivity.this, "진입4", Toast.LENGTH_SHORT).show();
